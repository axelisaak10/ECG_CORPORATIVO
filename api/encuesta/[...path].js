...
module.exports = async function handler(req, res) {
  ...
  // req.query.path viene del catch-all [...path].js
  // Ej: /api/encuesta/preguntas       -> path = ['preguntas']
  // Ej: /api/encuesta/preguntas/uuid  -> path = ['preguntas', 'uuid']
  // Ej: /api/encuesta/estadisticas    -> path = ['estadisticas']
  ...
  // ── PREGUNTAS ────────────────────────────────────────────────────────────
  if (route === 'preguntas') {
    // GET con id -> una pregunta (futuro uso)
  ...
  return res.status(404).json({ error: `Ruta no encontrada: ${route}` });
};
